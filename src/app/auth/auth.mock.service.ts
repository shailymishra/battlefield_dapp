...
export class AuthMockService {
        ...
        return of({}).pipe(
            map(() => {
                ...
                if (user) {
                    ...
                    this.getCurrentUserSubject.next(userDetails);
                } else {
        ...
        return of({}).pipe(
            map(() => {
                if (response.details) {
                    ...
                    this.isLoggedInSubject.next(true);
                    this.getCurrentUserSubject.next(response.details);
                }
                return response;
            })
        );
    ...
    changepassword(form) {
        ...
        /** API things */
        const userIndex = data.findIndex(user => user.email === loggedInUser.EMAIL_NOT_PRESENT);
